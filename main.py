...
def runGame():
	...
	pg.mixer.music.load('sound/title2.wav')
	...
	quitBtn = Button(setting, screen, "QUIT", 450)
	#make slector for buttons
	sel = Selector(setting, screen)
	sel.rect.x = playBtn.rect.x + playBtn.width + 15
	...
	aliens = Group()
	aliens2 = Group()
	gf.createFleet(setting, screen, ship, aliens, aliens2)
	...
	while runGame:
		...
		while stats.mainMenu:
			mm.checkEvents(setting, screen, stats, sb, playBtn, twoPlayBtn, aboutBtn, quitBtn, menuBtn, sel, ship, aliens, aliens2, bullets, eBullets)
			mm.drawMenu(setting, screen, sb, playBtn, menuBtn, twoPlayBtn, aboutBtn, quitBtn, sel)

		while stats.mainGame:
			#Game functions
			gf.checkEvents(setting, screen, stats, sb, playBtn, quitBtn, sel, ship, aliens, aliens2, bullets, eBullets) #Check for events
			if stats.gameActive:
				gf.updateAliens(setting, stats, sb, screen, ship, aliens, bullets, eBullets) #Update aliens
				gf.updateAliens2(setting, stats, sb, screen, ship, aliens2, bullets)
				gf.updateBullets(setting, screen, stats, sb, ship, aliens, aliens2, bullets, eBullets) #Update collisions
				ship.update() #update the ship
			gf.updateScreen(setting, screen, stats, sb, ship, aliens, aliens2, bullets, eBullets, playBtn, menuBtn, quitBtn, sel) #Update the screen

		while stats.mainAbout:
			About.checkEvents(setting, screen, stats, sb, playBtn, quitBtn, menuBtn, sel, ship, aliens, aliens2, bullets, eBullets)
